...
function getContentfulMode() {
	...
	if(stat && stat.isFile()) {
		mode = fs.readFileSync(localModePath, 'utf8').trim()
		return mode
	}
...
module.exports = function() {
	...
	return new Promise((resolve, reject) => {
		getContentfulKey(function(err, keyObj) {
			...
			if(!_contentfulClientCache[mode]) {
				...
				_contentfulClientCache[mode] = contentful.createClient(config)
			}

			resolve(_contentfulClientCache[mode])
