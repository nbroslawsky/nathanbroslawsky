	...
	let html = documentToHtmlString(fields.body, {
		renderNode: {
			[BLOCKS.EMBEDDED_ASSET]: (node) => {
				...
				return `<img class="content-img" src="${file.url}" title="${fields.title}" />`
			},
			[BLOCKS.EMBEDDED_ENTRY]: (node) => {
				...
				if(contentType == 'linkedImage') {
					...
					return (target.fields.url)
						? `<a href="${target.fields.url}"><img class="content-img" src="${file.url}" title="${imageFields.title}" /></a>`
						: `<img class="content-img" src="${file.url}" title="${imageFields.title}" />`
				}
